...
procedure TfrmFB_CSV.btnSelecBDClick(Sender: TObject);
var s:string;
begin
   clean;
   OpenDialog1.Filter := 'Bancos de Dados Firebird (*.gdb, *.fdb)|*.GDB;*.FDB';
   ...
   begin
      ...
      try
         ...
         begin
            ShowMessage('Não há tabelas para exportar/importar');
            clean;
         end
         else
            begin
...
begin
   try
      ...
      begin
         ShowMessage('Não há dados para exportar: a tabela está vazia.');
         FDTable1.Close;
         Exit;
      end;
   except
      ShowMessage('Tabela possui tipo(s) de campo(s) incompatível(veis) com o padrão FireDAC.');
      FDTable1.Close;
      Exit;
   end;
   ...
   begin
      ...
      Writeln(arqTXT, '|'); { '|' : fim de registro (poderia ser outro caracter desde que fosse do tipo "visível"
                              e diferente do delimitador de campos ';'}
   ...
   CloseFile(arqTXT);
   ShowMessage(cbbTabela.Text+'.txt'+' gravado com sucesso em '+ExtractFilePath(Application.ExeName));
   // Abre o Explorador de Arquivos na pasta onde foi gravado o arquivo .TXT
...
var s:string;
begin
   OpenDialog1.Filter := 'Arquivos Texto (*.txt)|*.txt';
   ...
   begin
      ...
      begin
         ShowMessage('Não é um arquivo texto válido.');
         edtCSV.Clear;
         btnImport.Enabled := False;
         Exit;
      end;
...
begin
   ...
   try
      ...
      if erro then
        ShowMessage('Não foi possível importar os dados.')
      else
        ShowMessage('Importação bem-sucedida.');
...
begin
  ...
  procedure TfrmFB_CSV.cbbTabelaExit(Sender: TObject);
begin
  FDTable1.TableName := cbbTabela.Text;
end;
